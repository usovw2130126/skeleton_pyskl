...
def pose_inference(args, frame_paths, det_results):
    ...
    for f, d in zip(frame_paths, det_results):
        ...
        # print(f)
        pose = inference_top_down_pose_model(model, frame_paths, d, format='xyxy')[0]
        ret.append(pose)
        # prog_bar.update()
        vis_frames = [
            vis_pose_result(model, frame_paths, pose)
            # for i in range(num_frame)
          ]
    return vis_frames,ret

def webcam_capture():
    args = parse_args()
    cap = cv2.VideoCapture(0)
    #Load the video, extract frames into ./tmp/video_name , osp.basename(osp.splitext("")[0]
    ...
    while True:
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            ...
            fake_anno = dict(
              ...
              modality='Pose',
              total_frames=1)
            det_results = detection_inference(args, frame_path)
            torch.cuda.empty_cache()
            vis,pose_results = pose_inference(args, frame_path, det_results)
            torch.cuda.empty_cache()
            if GCN_flag:
            ...
            # Current PoseC3D models are trained on COCO-keypoints (17 keypoints)
                num_keypoint = 17
                keypoint = np.zeros((num_person, 1, num_keypoint, 2),
                            dtype=np.float16)
                keypoint_score = np.zeros((num_person, 1, num_keypoint),
                                  dtype=np.float16)
            ...
            for x in vis:
              cv2.putText(x, action_label, (10, 30), FONTFACE, FONTSCALE,
